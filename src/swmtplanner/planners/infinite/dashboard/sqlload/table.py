...
class Table:
    ...
    def __init__(self, schema: TableSpec, cursor: Any, run_id: int) -> None:
        ...
        self._run_id = run_id
        self._display_cols = [
            c for c in schema.column_names if c != manifest.RUN_ID
        ]
        ...
        self._query = Query.build(cursor, run_id, schema.name)
    ...
    def _rebuild_query(self) -> None:
        ...
        self._query = Query.build(self._cursor, self._run_id, self.schema.name,
                                  **used_conds)
        self._chunk = None
        self._offset = 0
