...
class WorkCal(SwmtBase,
              read_only=('wd_start','wd_end','work_days','start'),
              priv=('holidays','cal_shift')):
    
    def __init__(self, business_hrs, work_days, holidays, start,
                 cal_shift = 0):
        wd_start, wd_end = business_hrs
        SwmtBase.__init__(self, _wd_start=wd_start, _wd_end=wd_end, _work_days=tuple(work_days),
                          _start=start, _holidays=set(holidays), _cal_shift=cal_shift)
