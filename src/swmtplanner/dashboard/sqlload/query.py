...
class Query:
    ...
    def build(
        ...
        **constraints: Filter | FKLookup,
        ...
        `FKLookup`'s join sub-query exposes `run_id`/`ref_col`, which can
        otherwise collide). Pagination needs a stable order, so the SELECT is
        `ORDER BY` the table's `order_columns` (its primary key, or the manifest's
        `order_by` for a key-less table)."""
        table = spec.name
        display_cols = [c for c in spec.column_names if c != RUN_ID]
        col_set = set(display_cols)

        joins: list[str] = []
        conds = [f'`{table}`.`{RUN_ID}` = {int(run_id)}']
